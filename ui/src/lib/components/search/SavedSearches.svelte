  ...
  {:else}
    <div class="space-y-2">
      {#each searches as search (search.id)}
        <div class="group relative border border-gray-200 rounded-md p-3 hover:bg-gray-50 cursor-pointer">
          <div on:click={() => handleLoad(search)}>
            ...
            <div class="mt-2 flex flex-wrap gap-1">
              {#if search.filters.title}
                <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-blue-100 text-blue-800">
                  タイトル: {search.filters.title.length > 10 ? search.filters.title.substring(0, 10) + '...' : search.filters.title}
                </span>
              {/if}
