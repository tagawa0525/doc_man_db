<script lang="ts">
  ...
  function getMetricColor(metric: string, value: number): string {
    switch (metric) {
      case 'fcp':
        return value > 3000 ? 'text-red-600' : value > 1800 ? 'text-yellow-600' : 'text-green-600';
      case 'lcp':
        return value > 4000 ? 'text-red-600' : value > 2500 ? 'text-yellow-600' : 'text-green-600';
      case 'fid':
        return value > 300 ? 'text-red-600' : value > 100 ? 'text-yellow-600' : 'text-green-600';
      case 'cls':
        return value > 0.25 ? 'text-red-600' : value > 0.1 ? 'text-yellow-600' : 'text-green-600';
      case 'memory':
...
<div class="fixed bottom-4 left-4 w-80 bg-white shadow-lg rounded-lg border border-gray-200 z-50">
    ...
    <div class="p-4">
      ...
      {#if isMonitoring}
        ...
        <div class="mb-4 p-3 bg-gray-50 rounded-md">
          <div class="flex items-center justify-between">
            <span class="text-sm font-medium text-gray-700">パフォーマンススコア</span>
            <span class="text-xl font-bold {getPerformanceScore() >= 80 ? 'text-green-600' : getPerformanceScore() >= 60 ? 'text-yellow-600' : 'text-red-600'}">
        ...
        <!-- Core Web Vitals -->
        <div class="mb-4">
          <h4 class="text-sm font-medium text-gray-900 mb-2">Core Web Vitals</h4>
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-600">FCP (First Contentful Paint)</span>
              <span class="text-xs font-medium {getMetricColor('fcp', metrics.fcp)}">
                {formatTime(metrics.fcp)}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-600">LCP (Largest Contentful Paint)</span>
              <span class="text-xs font-medium {getMetricColor('lcp', metrics.lcp)}">
                {formatTime(metrics.lcp)}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-600">FID (First Input Delay)</span>
              <span class="text-xs font-medium {getMetricColor('fid', metrics.fid)}">
                {formatTime(metrics.fid)}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-600">CLS (Cumulative Layout Shift)</span>
              <span class="text-xs font-medium {getMetricColor('cls', metrics.cls)}">
        ...
        <div class="mb-4">
          ...
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-600">TTFB (Time to First Byte)</span>
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-xs font-medium {getMetricColor('memory', metrics.memoryUsage)}">
