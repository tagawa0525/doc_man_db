<script lang="ts">
  ...
  import { auditAccessibility, checkColorContrast, createFocusTracker, type AccessibilityReport } from '$lib/utils/accessibility';
  ...
  const commonColors = [
    { name: 'プライマリボタン', fg: 'rgb(255, 255, 255)', bg: 'rgb(37, 99, 235)' },
    { name: 'セカンダリボタン', fg: 'rgb(55, 65, 81)', bg: 'rgb(243, 244, 246)' },
    { name: '成功メッセージ', fg: 'rgb(255, 255, 255)', bg: 'rgb(34, 197, 94)' },
    { name: 'エラーメッセージ', fg: 'rgb(255, 255, 255)', bg: 'rgb(239, 68, 68)' },
    { name: '警告メッセージ', fg: 'rgb(146, 64, 14)', bg: 'rgb(254, 240, 138)' },
    { name: '通常テキスト', fg: 'rgb(17, 24, 39)', bg: 'rgb(255, 255, 255)' },
    { name: 'サブテキスト', fg: 'rgb(107, 114, 128)', bg: 'rgb(255, 255, 255)' }
  ];
...
<div class="fixed top-4 right-4 w-96 bg-white shadow-lg rounded-lg border border-gray-200 z-50">
  <div class="p-4 border-b border-gray-200">
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-medium text-gray-900">アクセシビリティチェッカー</h3>
      <button
  ...
  <div class="p-4">
    ...
    {#if report}
      ...
      <div class="mb-4 p-3 bg-gray-50 rounded-md">
        ...
        <div class="mt-2">
          <div class="flex justify-between text-xs text-gray-600">
            <span>エラー: {report.issues.filter(i => i.severity === 'error').length}</span>
            <span>警告: {report.issues.filter(i => i.severity === 'warning').length}</span>
            <span>注意: {report.issues.filter(i => i.severity === 'notice').length}</span>
          </div>
        </div>
      </div>
      ...
      {#if showDetailedReport}
          ...
          {:else}
            <div class="space-y-2">
              {#each report.issues as issue}
                <div class="p-2 rounded-md text-xs {getSeverityColor(issue.severity)}">
        ...
        <div class="mb-4">
          <h4 class="text-sm font-medium text-gray-900 mb-2">色彩コントラスト</h4>
          <div class="space-y-1">
            {#each colorContrastResults as color}
              <div class="flex items-center justify-between text-xs">
                <span class="text-gray-700">{color.name}</span>
                <div class="flex items-center space-x-2">
                  <span class="font-mono">{formatContrastRatio(color.result.ratio)}</span>
                  <div class="flex space-x-1">
                    <span class="px-1 rounded text-xs {color.result.wcagAA ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
                      AA
                    </span>
                    <span class="px-1 rounded text-xs {color.result.wcagAAA ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
                      AAA
                    </span>
                  </div>
                </div>
      ...
      {:else}
        ...
        <div class="space-y-2 text-sm">
          ...
          {/if}
          {#if report.issues.length === 0}
            <div class="text-green-600">✅ 大きな問題は見つかりませんでした</div>
          {/if}
        </div>
      {/if}
    {/if}
    ...
    {#if !report && !isRunning}
      <div class="text-center text-gray-500 text-sm">
        「監査実行」ボタンをクリックして<br>アクセシビリティをチェックしてください
      </div>
    {/if}
  </div>
</div>