<script lang="ts">
  ...
  import { auditAccessibility, type AccessibilityReport } from '$lib/utils/accessibility';
  ...
  function trackInteraction(event: MouseEvent) {
    ...
    if (target.tagName === 'DIV' && !target.hasAttribute('role') && !target.onclick) {
      errorCount++;
  ...
  function trackKeyboardInteraction(event: KeyboardEvent) {
    ...
    if (activeElement && activeElement.tagName === 'BUTTON' && event.key === 'Enter') {
  ...
  function calculateUsabilityScore(): number {
    ...
    return Math.round((taskCompletionRate * 0.4) + (timeScore * 0.2) + (errorScore * 0.2) + (accessibilityScore * 0.2));
  }
...
<div class="fixed bottom-4 right-4 w-80 bg-white shadow-lg rounded-lg border border-gray-200 z-50">
    ...
    <div class="p-4 max-h-96 overflow-y-auto">
          ...
          <div class="space-y-2">
            {#each tasks as task}
              <div class="flex items-start space-x-2">
                ...
                />
                <div class="flex-1">
                  <div class="text-xs {task.completed ? 'text-green-600 line-through' : 'text-gray-700'}">
                    {task.description}
                  </div>
      ...
      <div class="mb-4">
        ...
        <div class="space-y-1 text-xs text-gray-600">
          ...
          <div>インタラクション時間: {formatTime(performanceMetrics.interactionTime)}</div>
          <div>レンダリング時間: {formatTime(performanceMetrics.renderTime)}</div>
        </div>
      </div>
      ...
      {#if accessibilityReport}
        <div class="mb-4">
          <h4 class="text-sm font-medium text-gray-900 mb-2">アクセシビリティ</h4>
          <div class="text-xs text-gray-600 mb-2">
            スコア: <span class="font-medium {accessibilityReport.score >= 80 ? 'text-green-600' : accessibilityReport.score >= 60 ? 'text-yellow-600' : 'text-red-600'}">
          ...
          {#if accessibilityReport.issues.length > 0}
            <div class="space-y-1">
              {#each accessibilityReport.issues.slice(0, 3) as issue}
                <div class="text-xs p-2 rounded {issue.severity === 'error' ? 'bg-red-50 text-red-700' : issue.severity === 'warning' ? 'bg-yellow-50 text-yellow-700' : 'bg-blue-50 text-blue-700'}">
      ...
      {#if !isRunning && completionTime > 0}
        <div class="p-3 bg-gray-50 rounded-md">
          ...
          <div class="text-2xl font-bold {calculateUsabilityScore() >= 80 ? 'text-green-600' : calculateUsabilityScore() >= 60 ? 'text-yellow-600' : 'text-red-600'}">
            {calculateUsabilityScore()}/100
          </div>
          <div class="text-xs text-gray-600 mt-1">
            完了率: {Math.round((tasks.filter(t => t.completed).length / tasks.length) * 100)}%
          </div>
        </div>
      {/if}
    </div>
