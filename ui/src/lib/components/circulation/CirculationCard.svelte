<script lang="ts">
  ...
  function getActionButtonText(action: 'Approve' | 'Reject' | 'RequestChanges'): string {
    switch (action) {
      case 'Approve': return '承認';
      case 'Reject': return '却下';
      case 'RequestChanges': return '修正依頼';
      default: return action;
    }
  }
</script>
  ...
  <div class="flex justify-between items-start mb-4">
    <div>
      <h3 class="text-lg font-medium text-gray-900">
        ステップ {step.step_number} - {getActionRequiredText(step.action_required)}
      </h3>
    ...
    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
      ...
      {step.status === 'Pending' ? '保留中' :
        step.status === 'Completed' ? '完了' : 'スキップ'}
    </span>
  </div>
