    ...
    <div class="flex items-center justify-between">
      ...
      {#if !isLoading}
        <div class="flex items-center space-x-2">
          <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {getOverallStatusColor()}">
            <svg class="mr-1 h-3 w-3" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d={getOverallStatusIcon()} />
            </svg>
    ...
    {:else}
          ...
          <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
            <div class="flex items-center space-x-3">
              <div class="flex-shrink-0">
                <svg class="h-5 w-5 {status.status === 'healthy' ? 'text-green-500' : status.status === 'warning' ? 'text-yellow-500' : status.status === 'error' ? 'text-red-500' : 'text-blue-500'}" 
                     fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" d={getStatusIcon(status.status)} />
                </svg>
              </div>
              <div>
                <h4 class="text-sm font-medium text-gray-900">{status.service}</h4>
            ...
            <div class="flex items-center space-x-4 text-sm">
              {#if status.responseTime}
                <div class="text-center">
                  ...
                  <p class="font-medium {status.responseTime > 1000 ? 'text-red-600' : status.responseTime > 500 ? 'text-yellow-600' : 'text-green-600'}">
              ...
              <div class="text-center">
                ...
                <p class="font-medium text-gray-900">{status.lastCheck.split(' ')[1]}</p>
              </div>
              
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {getStatusColor(status.status)}">
      ...
      <div class="mt-6 pt-4 border-t border-gray-200">
        <div class="flex items-center justify-between text-sm text-gray-500">
          ...
          <a href="/reports/system-status" class="text-blue-600 hover:text-blue-900 font-medium">
            詳細レポートを表示
          </a>
        </div>
      </div>
