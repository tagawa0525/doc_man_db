<script lang="ts">
  ...
  function getIconPath(iconName: string): string {
    const icons: Record<string, string> = {
      ...
      calendar: 'M6.75 3v2.25M17.25 3v2.25M3 9.75h18M12 15.75h.007v.008H12V15.75z',
      clock: 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z',
      default: 'M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5m.75-9l3-3 2.148 2.148A12.061 12.061 0 0116.5 7.605'
    };
...
<div class="relative overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:px-6 sm:py-6 hover:shadow-md transition-shadow">
  <dt>
  ...
  <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
    ...
    {#if change}
      <p class="ml-2 flex items-baseline text-sm font-semibold {getTrendColor(trend)}">
        {#if trend !== 'neutral'}
          <svg class="h-4 w-4 flex-shrink-0 self-center {trend === 'down' ? 'rotate-180' : ''}" 
