<script lang="ts">
  ...
  function setAutoCloseTimer() {
    if (autoClose) {
      const duration = notification.duration || defaultDuration[notification.type];
...
<div
  class="max-w-sm w-full border rounded-lg shadow-lg pointer-events-auto {getToastStyle(notification.type)}"
  ...
  <div class="p-4">
    <div class="flex">
      ...
      <div class="flex-shrink-0">
        <svg class="h-5 w-5 {getIconColor(notification.type)}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        {#if notification.actions && notification.actions.length > 0}
          <div class="mt-3 flex space-x-3">
            {#each notification.actions as action}
              <button
                type="button"
                class="text-sm font-medium {getTextColor(notification.type)} hover:opacity-75"
