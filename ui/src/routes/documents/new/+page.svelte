<script lang="ts">
  ...
  async function generateNumberPreview() {
    ...
    try {
      ...
      const month = String(new Date(formData.createdDate).getMonth() + 1).padStart(2, '0');
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      alert(`文書「${formData.title}」を作成しました。\n文書番号: ${generatedNumber}`);
  ...
  function getConfidentialityLabel(): string {
    const parts = [];
    if (formData.confidentiality.internalExternal === 'external') parts.push('社外');
    if (formData.confidentiality.importanceClass === 'class1') parts.push('重要');
    if (formData.confidentiality.personalInfo === 'present') parts.push('個人情報');
    return parts.length > 0 ? parts.join('・') : '通常';
  }
  
  function getConfidentialityColor(): string {
    if (formData.confidentiality.importanceClass === 'class1' || formData.confidentiality.personalInfo === 'present') {
              ...
              {#if generatedNumber}
                <div>
                  ...
                  <div class="px-3 py-2 bg-blue-50 border border-blue-200 rounded-md">
                    <span class="text-sm font-mono text-blue-800">{generatedNumber}</span>
                  </div>
                </div>
              {/if}
            </div>
          ...
          <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4">
                {#each internalExternalOptions as option}
                  <label class="flex items-center p-3 border rounded-md cursor-pointer hover:bg-gray-50
                                {formData.confidentiality.internalExternal === option.value ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}">
                    <input
                      type="radio"
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-4">
                {#each importanceClassOptions as option}
                  <label class="flex items-center p-3 border rounded-md cursor-pointer hover:bg-gray-50
                                {formData.confidentiality.importanceClass === option.value ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}">
                    <input
                      type="radio"
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-4">
                {#each personalInfoOptions as option}
                  <label class="flex items-center p-3 border rounded-md cursor-pointer hover:bg-gray-50
                                {formData.confidentiality.personalInfo === option.value ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}">
                    <input
                      type="radio"
            ...
            <div class="p-3 bg-gray-50 rounded-md">
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700">設定された機密レベル:</span>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {getConfidentialityColor()}">
        ...
        <div class="bg-white shadow rounded-lg p-6 sticky top-6">
          ...
          <div class="space-y-4 text-sm">
            ...
            <div>
              ...
              <p class="mt-1 font-mono text-blue-600">{generatedNumber || '（文書種別と作成日を選択してください）'}</p>
            </div>
            
            <div>
              <span class="font-medium text-gray-700">文書種別:</span>
              <p class="mt-1 text-gray-900">
                {documentTypeOptions.find(opt => opt.value === formData.documentTypeId)?.label || '（未選択）'}
              </p>
            </div>
            ...
            <div>
              ...
              <p class="mt-1 text-gray-900">{formData.createdDate || '（未入力）'}</p>
            </div>
            ...
            {#if formData.notes}
              <div>
                ...
                <p class="mt-1 text-gray-900 whitespace-pre-wrap">{formData.notes}</p>
              </div>
            {/if}
          </div>
        </div>
