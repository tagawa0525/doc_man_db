<script lang="ts">
  ...
  async function loadCirculations() {
    try {
      ...
      const pendingResponse = await fetch('/api/circulations/pending?user_id=1');
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="mt-2 text-gray-600">文書の回覧状況を確認し、承認手続きを行います。</p>
        </div>
        <Button 
    ...
    <div class="border-b border-gray-200 mb-6">
      <nav class="-mb-px flex space-x-8">
        ...
        <button
          class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'completed' 
            ? 'border-blue-500 text-blue-600' 
            : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
    ...
    {:else if activeTab === 'pending'}
      <div class="space-y-4">
        {#each pendingCirculations as step (step.id)}
          <CirculationCard 
            {step}
            on:action={handleStepAction}
          />
        {:else}
          <div class="text-center py-12">
            <div class="text-gray-400 text-lg mb-2">承認待ちの回覧はありません</div>
            <p class="text-gray-500">新しい回覧が開始されると、ここに表示されます。</p>
          </div>
        {/each}
      </div>
    {:else if activeTab === 'completed'}
      <div class="space-y-4">
            ...
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-sm text-gray-500 mt-2">
                  開始: {new Date(circulation.started_at).toLocaleDateString('ja-JP')}
                  {#if circulation.completed_at}
                    | 完了: {new Date(circulation.completed_at).toLocaleDateString('ja-JP')}
                  {/if}
                </p>
              </div>
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                {circulation.status === 'Completed' ? 'bg-green-100 text-green-800' :
                  circulation.status === 'Cancelled' ? 'bg-red-100 text-red-800' :
                  'bg-yellow-100 text-yellow-800'}">
                {circulation.status === 'Completed' ? '完了' :
                  circulation.status === 'Cancelled' ? 'キャンセル' : 'アクティブ'}
              </span>
            </div>
        ...
        {:else}
          <div class="text-center py-12">
            <div class="text-gray-400 text-lg mb-2">完了済みの回覧はありません</div>
          </div>
        {/each}
      </div>
    {/if}
