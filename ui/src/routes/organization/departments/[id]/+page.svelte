<script lang="ts">
  ...
  const mockDepartments: Record<string, any> = {
    '1': {
      ...
      description: 'システム開発・運用・保守を担当する部署です。\n社内のデジタル化推進と情報セキュリティの維持を主な業務としています。',
      budget: '50,000,000',
      location: '本社3F',
      ...
      kpis: [
        { name: 'システム稼働率', value: '99.9%', target: '99.5%', status: 'good' },
        { name: 'プロジェクト完了率', value: '95%', target: '90%', status: 'good' },
        { name: 'ユーザー満足度', value: '4.2/5.0', target: '4.0/5.0', status: 'good' },
        { name: 'セキュリティ事故件数', value: '0件', target: '0件', status: 'good' }
      ]
    },
    '2': {
      id: 2,
      ...
      kpis: [
        { name: 'コード品質スコア', value: '8.5/10', target: '8.0/10', status: 'good' },
  ...
  async function loadDepartment() {
    ...
    try {
      ...
      department = dept;
      employees = departmentEmployees[departmentId] || [];
    } catch (err) {
...
<div class="space-y-6">
  ...
  {:else if error}
    ...
    <div class="text-center py-12">
      ...
      <div class="mt-6">
        <Button variant="primary" on:click={loadDepartment}>
          再読み込み
        </Button>
      </div>
    </div>
      ...
      <div class="min-w-0 flex-1">
        <nav class="flex" aria-label="Breadcrumb">
          <ol class="flex items-center space-x-4">
            <li>
              <a href="/organization" class="text-gray-400 hover:text-gray-500">
            ...
            <li>
              <div class="flex items-center">
                ...
                <a href="/organization" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">組織管理</a>
              </div>
            </li>
            {#if department.parentName}
              <li>
                <div class="flex items-center">
                  ...
                  <span class="ml-4 text-sm font-medium text-gray-500">{department.parentName}</span>
                </div>
              </li>
            {/if}
            <li>
              <div class="flex items-center">
                ...
                <span class="ml-4 text-sm font-medium text-gray-500">{department.name}</span>
              </div>
            </li>
          </ol>
        </nav>
    ...
    <div class="bg-white shadow rounded-lg">
      <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
          <button
            class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'overview' 
          ...
          <button
            class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'members' 
              ? 'border-blue-500 text-blue-600' 
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          ...
          <button
            class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'kpis' 
              ? 'border-blue-500 text-blue-600' 
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
              <div>
                ...
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <dt class="text-sm font-medium text-gray-500">部署コード</dt>
                    <dd class="mt-1 text-sm text-gray-900">{department.code}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500">設立日</dt>
                    <dd class="mt-1 text-sm text-gray-900">{department.createdDate}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500">予算</dt>
                    <dd class="mt-1 text-sm text-gray-900">¥{department.budget}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500">電話番号</dt>
                    <dd class="mt-1 text-sm text-gray-900">{department.phoneNumber}</dd>
                  </div>
              ...
              <div>
                ...
                <p class="text-sm text-gray-900 whitespace-pre-wrap">{department.description}</p>
              </div>
              
              <div>
                <h4 class="text-sm font-medium text-gray-500 mb-3">主な業務・責任</h4>
                <ul class="space-y-2">
                  {#each department.responsibilities as responsibility}
                    <li class="flex items-start">
                      <svg class="mr-2 h-4 w-4 text-green-500 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="space-y-6">
              <div class="bg-gray-50 rounded-lg p-4">
                <h4 class="text-sm font-medium text-gray-900 mb-3">クイック情報</h4>
                <div class="space-y-3">
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-500">総人数</span>
                    <span class="text-sm font-medium text-gray-900">{department.employeeCount}名</span>
                  </div>
                  <div class="flex justify-between">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium text-gray-900">{department.location}</span>
                  </div>
                </div>
              </div>
              
              <div class="bg-blue-50 rounded-lg p-4">
                <h4 class="text-sm font-medium text-blue-900 mb-2">責任者情報</h4>
                <div class="flex items-center">
                  ...
                  <div class="ml-3">
                    <p class="text-sm font-medium text-blue-900">{department.managerName}</p>
                ...
                <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div class="space-y-2">
                    <div class="text-xs text-gray-600">
                      <span class="font-medium">メール:</span> {employee.email}
                    </div>
                    <div class="text-xs text-gray-600">
                      <span class="font-medium">入社日:</span> {employee.hireDate}
                    </div>
              ...
              {#each department.kpis as kpi}
                <div class="bg-white border border-gray-200 rounded-lg p-6">
                  <div class="flex items-center justify-between mb-4">
                    ...
                    <svg class="h-5 w-5 {getKpiStatusColor(kpi.status)}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={getKpiStatusIcon(kpi.status)} />
                    </svg>
                  </div>
                  ...
                  <div class="space-y-2">
                    <div class="flex justify-between items-center">
                      ...
                      <span class="text-lg font-semibold {getKpiStatusColor(kpi.status)}">{kpi.value}</span>
                    </div>
                    <div class="flex justify-between items-center">
                  ...
                  <div class="mt-4">
                    <div class="bg-gray-200 rounded-full h-2">
                      <div class="bg-{kpi.status === 'good' ? 'green' : kpi.status === 'warning' ? 'yellow' : 'red'}-500 h-2 rounded-full" 
                           style="width: {kpi.status === 'good' ? '90' : kpi.status === 'warning' ? '60' : '30'}%"></div>
                    </div>
                  </div>
                </div>
              {/each}
