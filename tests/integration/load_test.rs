...
async fn test_concurrent_document_creation() {
    ...
    println!("  - 成功率: {}/{} ({:.1}%)", successful_creations, CONCURRENT_DOCS, 
    ...
    println!("  - スループット: {:.1} docs/sec", CONCURRENT_DOCS as f64 / total_time.as_secs_f64());
    
    // パフォーマンス要件の検証
    assert!(avg_duration < Duration::from_millis(500), "平均レスポンス時間が500msを超えています");
    assert!(*max_duration < Duration::from_secs(2), "最大レスポンス時間が2秒を超えています");
    assert!(total_time < Duration::from_secs(10), "総実行時間が10秒を超えています");
}
...
async fn test_concurrent_search_operations() {
    ...
    // テスト用文書を事前に作成
    for i in 0..20 {
        test_env.create_document(CreateDocumentRequest {
    ...
    println!("  - 成功率: {}/{} ({:.1}%)", successful_searches, CONCURRENT_SEARCHES,
             (successful_searches as f64 / CONCURRENT_SEARCHES as f64) * 100.0);
    println!("  - 総実行時間: {:?}", total_time);
    println!("  - 平均レスポンス時間: {:?}", avg_duration);
    println!("  - 最大レスポンス時間: {:?}", max_duration);
    println!("  - 検索スループット: {:.1} searches/sec", CONCURRENT_SEARCHES as f64 / total_time.as_secs_f64());
    
    // パフォーマンス要件の検証（検索は高速であるべき）
    assert!(avg_duration < Duration::from_millis(200), "平均検索時間が200msを超えています");
    assert!(*max_duration < Duration::from_millis(1000), "最大検索時間が1秒を超えています");
}
...
async fn test_circulation_workflow_load() {
    ...
    for (i, doc) in documents.iter().enumerate() {
        let start = Instant::now();
        let result = test_env.start_circulation(StartCirculationRequest {
            document_id: doc.id,
            workflow_id: 1,
    ...
    for (i, circulation) in circulations.iter().enumerate() {
        let start = Instant::now();
        let result = test_env.approve_circulation_step(ApproveStepRequest {
    ...
    // パフォーマンス指標の計算と出力
    let avg_circulation_duration = circulation_durations.iter().sum::<Duration>() / circulation_durations.len() as u32;
    let avg_approval_duration = approval_durations.iter().sum::<Duration>() / approval_durations.len() as u32;
    ...
    println!("    - スループット: {:.1} workflows/sec", WORKFLOW_COUNT as f64 / circulation_total_time.as_secs_f64());
    println!("  - 承認処理:");
    println!("    - 総時間: {:?}", approval_total_time);
    println!("    - 平均時間: {:?}", avg_approval_duration);
    println!("    - スループット: {:.1} approvals/sec", WORKFLOW_COUNT as f64 / approval_total_time.as_secs_f64());
    
    // パフォーマンス要件の検証
    assert!(avg_circulation_duration < Duration::from_millis(300), "平均回覧開始時間が300msを超えています");
    assert!(avg_approval_duration < Duration::from_millis(200), "平均承認時間が200msを超えています");
}
...
async fn test_memory_usage_under_load() {
    ...
    for i in 0..OPERATIONS_COUNT {
        ...
        // 検索操作
        if i % 10 == 0 {
            let _search_result = test_env.search_documents(DocumentSearchInput {
        ...
        // 更新操作
        if i % 20 == 0 {
            let _updated = test_env.update_document(doc.id, UpdateDocumentRequest {
        ...
        // メトリクス記録
        test_env.metrics_service.record_request("/api/documents", start_time, true);
        test_env.metrics_service.record_database_query("INSERT", Duration::from_millis(10), true);
        test_env.metrics_service.record_cache_operation("get", i % 2 == 0);
    }
    ...
    println!("  - 総実行時間: {:?}", total_time);
    println!("  - メモリ使用量:");
    println!("    - Heap使用: {:.1} MB", metrics.memory_usage.heap_used as f64 / (1024.0 * 1024.0));
    println!("    - Heap総量: {:.1} MB", metrics.memory_usage.heap_total as f64 / (1024.0 * 1024.0));
    println!("    - RSS: {:.1} MB", metrics.memory_usage.rss as f64 / (1024.0 * 1024.0));
    println!("  - パフォーマンス:");
    println!("    - RPS: {:.1}", metrics.performance.requests_per_second);
    println!("    - 平均レスポンス時間: {:.1}ms", metrics.performance.average_response_time_ms);
    println!("    - エラー率: {:.1}%", metrics.performance.error_rate * 100.0);
    println!("  - データベース:");
    println!("    - 総クエリ数: {}", metrics.database.total_queries);
    println!("    - 遅いクエリ数: {}", metrics.database.slow_queries);
    println!("    - 平均クエリ時間: {:.1}ms", metrics.database.average_query_time_ms);
    println!("  - キャッシュ:");
    ...
    // システム健全性の検証
    assert_eq!(health_status.overall, doc_man_db::services::HealthState::Healthy, "システムが健全状態ではありません");
    assert!(metrics.performance.error_rate < 0.01, "エラー率が1%を超えています");
    assert!(metrics.memory_usage.heap_used < 500 * 1024 * 1024, "ヒープ使用量が500MBを超えています");
}
...
async fn test_sustained_load_endurance() {
    ...
    println!("⏱️  持続負荷テスト開始 ({}秒間)...", TEST_DURATION.as_secs());
    ...
    while start_time.elapsed() < TEST_DURATION {
        ...
        for i in 0..OPERATIONS_PER_SECOND {
            ...
            let result = test_env.create_document(CreateDocumentRequest {
                ...
                created_date: NaiveDate::from_ymd_opt(2024, 12, 15).unwrap(),
                confidentiality: TestConfidentiality::default(),
                notes: Some(format!("持続テスト {} 秒目", start_time.elapsed().as_secs())),
            }, &admin_token).await;
            ...
            if op_start.elapsed() > Duration::from_millis(500) {
                println!("⚠️  操作 {} のレスポンス時間が500msを超えました: {:?}", operation_count, op_start.elapsed());
            }
        }
        ...
        // 進捗表示（10秒ごと）
        if start_time.elapsed().as_secs() % 10 == 0 && cycle_start.elapsed() < Duration::from_millis(100) {
            println!("  📊 {}秒経過 - 操作数: {}, エラー数: {}", 
                     start_time.elapsed().as_secs(), operation_count, error_count);
        }
    }
    ...
    assert!(avg_throughput >= OPERATIONS_PER_SECOND as f64 * 0.8, "平均スループットが期待値の80%を下回りました");
}