...
async fn test_permission_enforcement() {
    ...
    // 制限ユーザーでのアクセス（失敗すべき）
    let limited_access_result = test_env.get_document(confidential_doc.id, &limited_token).await.unwrap();
    assert!(limited_access_result.is_none());
    
    // 管理者でのアクセス（成功すべき）
    let admin_access_result = test_env.get_document(confidential_doc.id, &admin_token).await.unwrap();
...
async fn test_document_search_functionality() {
    ...
    assert!(search_results.documents.len() >= 1);
    assert!(search_results.documents.iter().any(|d| d.title.contains("技術")));
...
async fn test_document_lifecycle_with_updates() {
    ...
    assert_eq!(updated_document.notes, Some("更新履歴：タイトルを変更しました".to_string()));
    ...
    assert!(search_results.documents.len() >= 1);
    let found_doc = search_results.documents.iter().find(|d| d.id == original_id).unwrap();
...
async fn test_multiple_users_concurrent_operations() {
    ...
    // 並行して文書を作成
    let admin_doc_future = test_env.create_document(CreateDocumentRequest {
    ...
    let user_doc_future = test_env.create_document(CreateDocumentRequest {
    ...
    assert!(search_results.documents.len() >= 2);
    assert!(search_results.documents.iter().any(|d| d.title == "管理者文書"));
    assert!(search_results.documents.iter().any(|d| d.title == "ユーザー文書"));
}