...
fn test_document_validation_errors() {
    ...
    assert_eq!(
        invalid_type_request.validate(),
        Err(DocumentValidationError::InvalidDocumentTypeId)
    );
    ...
    assert_eq!(
        invalid_creator_request.validate(),
        Err(DocumentValidationError::InvalidCreatedBy)
    );
}
