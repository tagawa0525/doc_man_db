...
pub async fn find_all_duplicates(
    ...
    info!("Finding all types of duplicates with threshold: {}", threshold);
...
pub async fn bulk_merge_data(
    ...
    info!("Starting bulk merge operation for {} requests by user {}", requests.len(), user_id);
...
pub async fn get_merge_history(
    ...
    let history = deduplication_service
        .get_merge_history(limit)
        .await?;
