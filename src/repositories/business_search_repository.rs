...
impl BusinessSearchRepository for SqliteBusinessSearchRepository {
    async fn search_businesses_advanced(
        ...
        if !user_permissions.can_view_all_businesses {
            joins.push(
                "LEFT JOIN business_members bm_perm ON b.id = bm_perm.business_id"
            );
        ...
        let count_query = format!(
            "SELECT COUNT(DISTINCT b.id) as count FROM businesses b {} {}",
            ...
            BusinessSearchField::CustomerName => {
                ...
                for row in rows {
                    ...
                    suggestions.push(AutocompleteSuggestion {
                        ...
                        label: format!("{} ({} 件)", customer_name, business_count),
    ...
    async fn get_employee_businesses(
        ...
        let businesses = rows.into_iter()
            .map(|row| {
                ...
                let is_active = member_end_date.map_or(true, |end| end >= chrono::Utc::now().date_naive());
    ...
    async fn get_business_search_aggregations(
        ...
        let customer_counts = customer_rows.into_iter()
        ...
        let member_counts = member_rows.into_iter()
            .map(|row| MemberCount {
        ...
        let year_counts = year_rows.into_iter()
            .map(|row| YearCount {
