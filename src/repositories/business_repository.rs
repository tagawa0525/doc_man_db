...
use crate::repositories::document_repository::RepositoryError;
use crate::models::{
    ...
    UpdateBusinessRequest, BusinessSearchFilters, ExternalContact
};
...
pub trait BusinessRepository: Send + Sync {
    ...
    async fn update_member(&self, member_id: i32, member: UpdateBusinessMemberRequest) -> Result<BusinessMember, BusinessError>;
...
impl BusinessRepository for SqliteBusinessRepository {
    ...
    async fn update(&self, id: i32, request: UpdateBusinessRequest) -> Result<Business, BusinessError> {
    ...
    async fn update_member(&self, member_id: i32, request: UpdateBusinessMemberRequest) -> Result<BusinessMember, BusinessError> {
        let mut set_clauses = Vec::new();
        let mut bind_values = Vec::new();
        ...
        if set_clauses.is_empty() {
            return self.get_member_by_id(member_id).await?
                .ok_or(BusinessError::NotFound);
        }
    ...
    async fn generate_business_number(&self) -> Result<String, BusinessError> {
        ...
        Ok(format!("BUS-{}{:03}", current_year, next_number))
    }
}