...
impl DataCleanupService {
    ...
    pub async fn run_cleanup(&self, execution_id: Uuid) -> Result<BatchExecution, BatchError> {
        ...
        statistics.cleanup_duration_minutes = 
            execution.end_time.unwrap().signed_duration_since(execution.start_time)
                .num_minutes() as f64;
    ...
    pub async fn run_manual_cleanup(
        ...
        info!("Starting manual cleanup by user {}: {:?}", started_by, cleanup_types);
        ...
        info!("Manual cleanup completed: {} tasks processed", results.len());
        Ok(results)
    }
