use crate::batch::{AdSyncService, DataCleanupService, FileCheckService};
...
impl BatchScheduler {
    ...
    pub async fn run_batch_manually(
        ...
        info!(
            "Starting manual batch execution: {:?} by user {}",
            batch_type, started_by
        );
        ...
        match batch_type {
            ...
            _ => Err(BatchError::JobExecution(
                "Batch type not supported".to_string(),
            )),
        }
    }
...
async fn run_monthly_file_check(
    ...
    if result.error_count > 0 {
        notification_service
            .send_file_check_alert(&result, "Monthly file check completed with errors")
            .await
            .map_err(|e| BatchError::JobExecution(e.to_string()))?;
    } else {
        notification_service
            .send_file_check_summary(&result)
            .await
            .map_err(|e| BatchError::JobExecution(e.to_string()))?;
    }
...
async fn run_monthly_maintenance(
    ...
    // メンテナンス通知
    notification_service
        .send_maintenance_notification(&result)
        .await
        .map_err(|e| BatchError::JobExecution(e.to_string()))?;
