...
impl QueryRoot {
    ...
    /// Search documents
    async fn search_documents(&self, ctx: &Context<'_>, filters: DocumentSearchFilters) -> Result<SearchDocumentsResult> {
        let state = ctx.data::<AppState>()?;
        let search_filters = filters.into();
        
        match state.document_handlers.search_documents(search_filters).await {
            Ok((documents, total)) => {
                let graphql_documents: Vec<Document> = documents.into_iter().map(|d| d.into()).collect();
...
impl MutationRoot {
    ...
    async fn create_document(&self, ctx: &Context<'_>, input: CreateDocumentInput) -> Result<CreatedDocumentWithNumber> {
        let state = ctx.data::<AppState>()?;
