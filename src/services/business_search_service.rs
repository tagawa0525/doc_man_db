...
use crate::models::{
    ...
    BusinessSearchResult, EmployeeBusinesses,
};
...
impl BusinessSearchService {
    ...
    pub async fn get_business_statistics(
        ...
        let my_businesses = if user_permissions.can_view_all_businesses {
            ...
            self.count_my_businesses(user_permissions.employee_id, user_permissions)
                .await?
        };
    ...
    pub async fn get_search_suggestions(
        ...
        let mut suggestions = BusinessSearchSuggestions {
            recent_customers: self.get_recent_customers(user_permissions).await?,
            popular_searches: self.get_popular_searches(user_permissions).await?,
            ..Default::default()
        };

        // 自分が関与している業務
        if !user_permissions.can_view_all_businesses {
            suggestions.my_recent_businesses = self
                .get_my_recent_businesses(user_permissions.employee_id, user_permissions)
                .await?;
        }
    ...
    async fn count_total_businesses(
        &self,
    ...
    async fn count_active_businesses(
        &self,
        _user_permissions: &UserPermissions,
    ) -> Result<i32, SearchError> {
        // TODO: 実際のカウント実装
    ...
    async fn count_completed_this_month(
        &self,
        _user_permissions: &UserPermissions,
    ) -> Result<i32, SearchError> {
        // TODO: 実際のカウント実装
    ...
    async fn get_my_recent_businesses(
        &self,
        _employee_id: i32,
        _user_permissions: &UserPermissions,
    ) -> Result<Vec<String>, SearchError> {
...
mod tests {
    ...
    fn test_normalize_business_name() {
        assert_eq!(
            normalize_business_name("  システム  開発  "),
            "システム 開発"
        );
