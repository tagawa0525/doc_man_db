...
| F001d | 連番管理                 | システムは各ルール・年月・部署の組み合わせで独立した連番を管理し、指定桁数（2〜5桁）でゼロ埋め番号を生成する   |
...
| F007a | AD同期・監視     | システムは定期的にActive Directoryを監視し、ユーザー追加・削除を検知してローカルデータベースを更新する |
...
| F010b | 承認書要否制御   | システムは時期・部署・文書種別の組み合わせに応じて承認書の必要性を判定する                                |
...
| F015b | 機密レベル定義 | システムは各軸をenum型で定義し日本語名と1:1対応させる（社内外：社内/社外、重要度：低/中/高、個人情報：無/有） |
